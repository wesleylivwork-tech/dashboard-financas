...
// uso: node /root/bot-iara/auditor.mjs         (corrige o obvio em silencio e escreve o relatorio no Notion)
...
const PAG_RELATORIO = "3b9b9d483a748177a18ac5c13f80bbab";
...
// escreve o relatorio na pagina do Notion, apagando o anterior. NAO manda nada no Telegram.
async function publicaRelatorio(linhas) {
  if (DRY) return;
  try {
    const r = await fetch(`https://api.notion.com/v1/blocks/${PAG_RELATORIO}/children?page_size=100`, { headers: H });
    const j = await r.json();
    for (const b of (j.results || [])) {
      await fetch("https://api.notion.com/v1/blocks/" + b.id, { method: "DELETE", headers: H });
      await dorme(120);
    }
    const blocos = linhas.slice(0, 95).map(t => ({
      object: "block", type: "paragraph",
      paragraph: { rich_text: [{ type: "text", text: { content: String(t).slice(0, 1900) } }] }
    }));
    await fetch(`https://api.notion.com/v1/blocks/${PAG_RELATORIO}/children`, {
      method: "PATCH", headers: H, body: JSON.stringify({ children: blocos })
    });
  } catch (e) { console.log("falhou ao publicar relatorio:", e.message); }
}
...
console.log(msg);
const linhas = [`Ultima auditoria: ${new Date().toISOString().slice(0, 16).replace("T", " ")} UTC`, ""];
linhas.push(`Corrigidos sozinho: ${corrigidos.length}`);
corrigidos.forEach(c => linhas.push("- " + c));
linhas.push("");
linhas.push(`Pra voce olhar: ${suspeitos.length}`);
suspeitos.forEach(s2 => linhas.push("- " + s2));
if (!corrigidos.length && !suspeitos.length) linhas.push("Nada duplicado. Bases limpas.");
await publicaRelatorio(linhas);
