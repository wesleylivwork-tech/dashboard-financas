...
if "(V6)" in s:
    ...
    # C) indice da ultima mensagem de cada chat no lote (fora album de fotos)
    m = re.search(r"^(\s*)const _ultGrupo = \{\};[^\n]*$", s, re.M)
    if not m:
        print("ERRO: nao achei 'const _ultGrupo'"); sys.exit(1)
    ind = m.group(1)
    ultTxt = ("\n" + ind + "const _ultTxt = {}; // (V6) ultima msg de cada chat no lote\n"
              + ind + "upd.result.forEach((x2, i2) => { const m2 = x2.message; "
              "if (m2 && m2.chat && !m2.media_group_id) _ultTxt[m2.chat.id] = i2; });")
    s = s[:m.end()] + ultTxt + s[m.end():]

    # D) junta os textos: so responde na ultima mensagem do chat naquele lote
    m = re.search(r"^(\s*)const _g = msg\.media_group_id;", s, re.M)
    if not m:
        print("ERRO: nao achei 'const _g = msg.media_group_id'"); sys.exit(1)
    ind = m.group(1)
    junta = (
        ind + "// (V6) rajada: acumula as mensagens seguidas e responde uma vez so\n"
        + ind + "if (!msg.media_group_id) {\n"
        + ind + "  const _soTexto = !!msg.text && !_img;\n"
        + ind + "  if (_soTexto && _ultTxt[chatId] !== _i) { bufTxt[chatId] = (bufTxt[chatId] || []).concat(_raw); continue; }\n"
